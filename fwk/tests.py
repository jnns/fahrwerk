...
from .templatetags import fwk_extras
...
class TemplateTagTest(TestCase):
    def test_holidays(self):
        """
        Test if holidays are returned as list of ``datetime.date`` objects
        from remote backend.
        """
        holidays = fwk_extras.get_holidays()
        self.assertTrue(
            all([isinstance(h, date) for h in holidays])
        )

    def test_warning_on_holiday(self):
        """
        Test if warning is displayed on a holiday.
        """
        config.FWK_DAYS_CLOSED = [date.today()]
        context = fwk_extras.opening_hours_reminder()
        self.assertFalse(context["now_open"])
